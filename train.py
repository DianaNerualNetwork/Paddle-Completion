...
from loss_funcs.smoothness import SmoothnessLoss
...
from models.fcfrnet import DepthComletionNet
...
configs.use_pose = ("photo" in configs.train_mode)
# args.pretrained = not args.no_pretrained
configs.result = os.path.join('..', 'results')
configs.use_rgb = ('rgb' in configs.dataset["input_mode"]) or configs.use_pose
configs.use_d = 'd' in configs.dataset["input_mode"]
configs.use_g = 'g' in configs.dataset["input_mode"]
if configs.use_pose:
    configs.w1, configs.w2 = 0.1, 0.1
else:
    configs.w1, configs.w2 = 0, 0
print(configs)

if configs.use_pose:
    # hard-coded KITTI camera intrinsics
    K = load_calib(configs.dataset['calib_path'])
...
def FCFRNet_train(args):
    ...
    optim=paddle.optimizer. Adam( learning_rate=args.optimizer['lr'],parameters=model_named_params, weight_decay=args.optimizer['weight_decay'],)
    print("completed.")
    ...
    if is_eval:
        ...
        result, is_best = iterate("val", args, val_loader, model, None, _logger,
                                  args.epochs)
        return
